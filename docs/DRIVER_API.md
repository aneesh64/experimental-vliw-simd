...
// Write first bundle (8 words for 256-bit baseline bundle, little-endian)
for (int i = 0; i < BUNDLE_WORDS; i++) {
    *imwd = program_data[i];  // Word 0..7; hardware commits the bundle after word 7
    // and auto-increments IMBAS
}

// Next bundle at address 1
for (int i = 0; i < BUNDLE_WORDS; i++) {
    *imwd = program_data[BUNDLE_WORDS + i];
}
```
...
// Simplified program: initialize s[0]=0, increment by 1, store to mem[s[0]], halt
// Generated by tools/assembler.py with baseline Sim config (bundle_width=256 bits, 8 words/bundle)
//
// Program semantics:
//   Bundle 0: CONST — s[0] = 0
//   Bundle 1: ADD_IMM — s[0] = s[0] + 1  (NOP inserted for RAW latency)
//   Bundle 2: STORE — mem[s[0]] = s[0]    (stores value=1 at word address 1)
//   Bundle 3: HALT
//
// Each bundle = 256 bits = 8 × 32-bit words (little-endian, LSB-first field order)
const uint32_t program[] = {
    // Bundle 0: CONST s[0] = 0 (LOAD slot active, all others NOP)
    0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00050000, 0x00000000, 0x00000000, 0x00000000,

    // Bundle 1: ADD_IMM s[0] = s[0] + 1 (FLOW slot active, others NOP; also NOP
    //           for CONST RAW latency gap filled by scheduler)
    0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00010000, 0x18000000, 0x00000001,

    // Bundle 2: STORE mem[s[0]] = s[0] (STORE slot active)
    0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00009000, 0x00000000, 0x00000000,

    // Bundle 3: HALT (FLOW slot: valid=1, opcode=HALT=4)
    0x00000000, 0x00000000, 0x00000000, 0x00000000,
    0x00000000, 0x00000000, 0x20000000, 0x00000001,
};
// 4 bundles × 8 words = 32 words total
#define BUNDLE_WORDS 8   /* bundle_width / 32 — must match RTL config */
...
## 10. Matrix Tiled Artifact Flow Example

For the tiled DSL `32x32` matrix-matmul path, the repository now includes:
- artifact generator: [tools/matrix_matmul_32x32_demo.py](../tools/matrix_matmul_32x32_demo.py)
- driver-side example consumer: [drivers/example_matrix_matmul_32x32.c](../drivers/example_matrix_matmul_32x32.c)

Suggested workflow:

```bash
python tools/matrix_matmul_32x32_demo.py --emit-dir artifacts/matrix32
```

This emits:
- `matrix_matmul_32x32_imem_words.txt`
- `matrix_matmul_32x32_lhs_tiles_u8.bin`
- `matrix_matmul_32x32_rhs_tiles_u8.bin`
- `matrix_matmul_32x32_expected_out_tiles_u32_le.bin`
- `matrix_matmul_32x32_expected_out_row_major_u32_le.bin`
- `matrix_matmul_32x32_metadata.json`
...
1. **IMEM load:** 8 words per bundle (256-bit baseline); ~1 µs per word over AXI (assumes 100 MHz clock)
